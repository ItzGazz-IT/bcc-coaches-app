...
function PlatformClubs({ app }) {
  const { clubs, teams, coaches, createClubWithAdmin, deleteClub } = app
  const [form, setForm] = useState({ name: "", code: "", contactPerson: "", email: "", phone: "", region: "", activeSeason: String(new Date().getFullYear()), adminName: "", adminEmail: "", password: "", enabledSports: SPORTS.map(s => s.id) })
  ...
  const createWorkspace = async event => {
    ...
    try {
      const result = await createClubWithAdmin(
        { name: form.name.trim(), code: form.code.trim().toUpperCase(), contactPerson: form.contactPerson.trim(), email: form.email.trim(), phone: form.phone.trim(), region: form.region.trim(), activeSeason: form.activeSeason.trim(), enabledSports: form.enabledSports },
        { fullName: form.adminName.trim(), email: form.adminEmail.trim(), password: form.password }
      )
      setResult({ club: form.name.trim(), email: result.email, password: form.password })
      setForm({ name: "", code: "", contactPerson: "", email: "", phone: "", region: "", activeSeason: String(new Date().getFullYear()), adminName: "", adminEmail: "", password: "", enabledSports: SPORTS.map(s => s.id) })
  ...
  return <div className="platform-page min-h-screen p-4 md:p-8">
    <div className="max-w-6xl mx-auto space-y-7">
      ...
      {result && <div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-4 md:p-5 flex flex-col md:flex-row md:items-center gap-4"><CheckCircle2 className="text-emerald-600 shrink-0" /><div className="flex-1"><h3 className="font-black text-emerald-950">{result.club} is ready for onboarding</h3><p className="text-sm text-emerald-800">Share these temporary credentials securely with the club administrator and ask them to reset the password after signing in.</p></div><div className="rounded-xl bg-white/80 px-4 py-2 text-xs font-mono"><p>{result.email}</p><p>{result.password}</p></div><button type="button" onClick={() => navigator.clipboard?.writeText(`UNYRA club login\nEmail: ${result.email}\nTemporary password: ${result.password}\nSign in: https://unyra.kodeiq.online`)} className="text-xs font-black text-emerald-800 flex items-center gap-1"><Copy size={14} /> Copy details</button></div>}
      ...
      <div>
        <form onSubmit={createWorkspace} className="ops-card ops-card-primary p-6 md:p-8 space-y-7">
          ...
          <div className="border-t border-slate-100 pt-7"><div className="flex items-start gap-4 mb-4"><span className="workspace-step">02</span><div><h3 className="font-black flex items-center gap-2"><Shield size={17} className="text-cyan-600" /> First club administrator</h3><p className="text-xs text-slate-500 mt-1">A secure Firebase account for completing onboarding.</p></div></div><div className="grid md:grid-cols-3 gap-4"><Field label="Administrator name" required><input className={field} value={form.adminName} onChange={e => set("adminName", e.target.value)} placeholder="Full name" required /></Field><Field label="Administrator email" required><input className={field} type="email" value={form.adminEmail} onChange={e => set("adminEmail", e.target.value)} placeholder="admin@club.co.za" required /></Field><Field label="Temporary password" required hint="At least 8 characters"><input className={field} type="password" value={form.password} onChange={e => set("password", e.target.value)} placeholder="Create secure password" minLength={8} required /></Field></div></div>
...
function ClubOnboarding({ app }) {
  ...
  const [coach, setCoach] = useState({ fullName: "", email: "", password: "", teamId: "" })
  ...
  const createCoach = async e => { e.preventDefault(); if (!coach.fullName || !coach.email || !coach.password || !coach.teamId) return; await addCoach({ ...coach, clubId: club.id, teamIds: [coach.teamId], role: "coach" }); setCoach({ fullName: "", email: "", password: "", teamId: "" }) }
